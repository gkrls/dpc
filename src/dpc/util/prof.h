...
public:
  ...
  uint64_t get_tsc_hz() {
    ...
    // sleep briefly to calibrate
    std::this_thread::sleep_for(std::chrono::milliseconds(50));
    uint64_t tsc1 = __rdtsc();
    auto t1 = std::chrono::steady_clock::now();
    double secs = std::chrono::duration<double>(t1 - t0).count();
    return static_cast<uint64_t>((tsc1 - tsc0) / secs);
  }

  void summary(bool json = false) {
    const uint64_t hz = get_tsc_hz();
    auto const to_us = [&](uint64_t cyc) -> uint64_t {
      return (uint64_t)(((__uint128_t)cyc * 1000000ull + hz / 2) / hz);
    };
    ...
    fmt::println("'{}': {{'ops': {}, 'skipped': {}, 'rx': {}, 'rx_k': {},  'drain': {:.2f}, 'burst': {:.2f}, "
                 ...
                 tid, ops, ops_skipped, pkt.rx_res, pkt.rx_res_k, t_drain.mean, t_burst.mean, t_rtt0.n, t_rtt0.min,
                 t_rtt0.max, t_rtt0.mean, t_rtt0.sd, t_rtt0.p50, t_rtt0.p95, t_rtt0.p99, t_rtt_init.n, t_rtt_init.min,
                 t_rtt_init.max, t_rtt_init.mean, t_rtt_init.sd, t_rtt_init.p50, t_rtt_init.p95, t_rtt_init.p99,
                 t_rtt.n, t_rtt.min, t_rtt.max, t_rtt.mean, t_rtt.sd, t_rtt.p50, t_rtt.p95, t_rtt.p99);
  }
};
