#ifndef DPC_UTIL_BITSET_H
#define DPC_UTIL_BITSET_H
...
template <size_t N> struct Bitset {
  static_assert(N == 64 || N == 128 || N == 256 || N == 512 || N == 1024 || N == 2048,
                "Bitset only supports 64,128,256,512,1024 or 2048 bits");

private:
  // using T = std::conditional_t<N == 32, uint32_t, uint64_t>;
  static constexpr size_t words = (N + 63) / 64; // 1 for 64, 2 for 128, 4 for 256
  uint64_t bits[words] = {};
  ...
  inline std::string bitstring() const {
    ...
    if constexpr (N <= 64) {
      ...
      for (size_t i = N; i > 0; --i) {
        size_t word = (i - 1) / 64; // bits[0] is least significant
        size_t bit = (i - 1) % 64;
